...
DWORD CBY_Input::KeyCheck(DWORD inKey)
{
...
bool CBY_Input::GetKeyCheck(DWORD inkey)
{
...
bool CBY_Input::GetKeyCheckDouble(DWORD inkey,float CheckTime)
{
	if (KeyCheck(inkey))																//방향키로 캐릭터의 이동을 정의 계산하며 사양의 차이에 불공정함이 생기지 않게 시간의 동기화를 이용
	{
...
bool CBY_Input::Frame()
{
	GetCursorPos(&m_Mouse);
	ScreenToClient(g_hwnd, &m_Mouse);
	return true;
}
bool CBY_Input::Render()
{
	return true;
}
bool CBY_Input::Release()
{
	return true;
}
