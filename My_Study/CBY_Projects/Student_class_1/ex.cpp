...
//void Name();				//이름순 정렬
//void Num();					//성적순 정렬
//StuNode* reswap(int a);				//노드의 주소값 교환을 위해 재귀함수로 주소 맞추는 용도
//StuNode* make();				//노드 새로 만들기

LinkList Link;		//링크리스트 호출
StuNode* Head=Link.getHead();
StuNode* Tail=Link.getTail();
Control Con;		//컨트롤 클래스 호출
...
//	int NodeNum;
//	StuNode* Node = Link_Head->m_next;
//	StuNode* NodeComparison = Node->m_next;	//노드와 비교하기 위한 선언 노드의 다음 노드를 대입한다
//	StuNode sw;
//
//
...
//	int NodeNum;
//	StuNode* Node = Link_Head->m_next;
//	StuNode* NodeComparison = Node->m_next;
//	StuNode sw;
//
//
//	NodeNum = NodeNumber();			//노드 개수를 측정하는 함수를 이용하여 개수 측정후 대입
...
//
//StuNode* reswap(int a)
//{
//	StuNode* rere;
//	if (a == 0)
//	{
//		StuNode* rere = Link_Head;
//		return rere;
//	}
//	rere = reswap(a - 1);
//	return rere->m_next;
