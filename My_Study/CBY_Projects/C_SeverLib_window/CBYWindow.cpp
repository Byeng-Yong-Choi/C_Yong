...
bool CBYWindow::InitWindow(const TCHAR* WindowClass,const TCHAR* WindowName)
{
	...
	GetClientRect(C_hwnd, &Winrt);																		//생성된 윈도우의 크기를 핸들을 통해 Winrt에 입력받는다.
	g_hwnd = C_hwnd;																					//생성된 윈도우 핸들을 다른 클래스와 함께 쓰기위해 전역으로 받는중
	ShowWindow(C_hwnd, SW_SHOW);																		//윈도우는 생성되었다고 아니기에 윈도우를 호출해서 화면에 띄우는 함수.
	return true;
}

CBYWindow::CBYWindow()
{
	g_CBYWindow = this;																					//윈도우에 생기는 이벤트를 전달하고 메세지를 처리하기 위해 WndProc함수가 쓰이는데
