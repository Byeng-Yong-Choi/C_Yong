...
namespace CDX
{
	ID3D11SamplerState*  CDxState::g_pSampler = nullptr;

	ID3D11BlendState* CDxState::g_pAlpahBlend = nullptr;
	ID3D11BlendState* CDxState::g_pAlpahBlendDisable = nullptr;

	ID3D11RasterizerState* CDxState::g_pRSSold = nullptr;
	ID3D11RasterizerState* CDxState::g_pRSWireFrame = nullptr;

	ID3D11DepthStencilState* CDxState::g_pDSS = nullptr;
	ID3D11DepthStencilState* CDxState::g_pDSSDisable = nullptr;


	//Tool
	Microsoft::WRL::ComPtr<ID3D11SamplerState> CDxState::g_pSSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11BlendState>	CDxState::g_pBSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11RasterizerState> CDxState::g_pRSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11DepthStencilState> CDxState::g_pDSSEdit = 0;

	D3D11_SAMPLER_DESC CDxState::g_SamplerDesc;
	D3D11_BLEND_DESC CDxState::g_BlendDesc = { 0, };
	D3D11_RASTERIZER_DESC CDxState::g_RasterizerDesc;
	D3D11_DEPTH_STENCIL_DESC CDxState::g_DepthDesc = { 0, };


	HRESULT CDxState::SetState(ID3D11Device* pd3dDevice)
	{
		HRESULT hr;
	...
	HRESULT CDxState::SetSamplerState(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pContext, D3D11_SAMPLER_DESC* pDesc, UINT StartSlot, UINT NumSamplers)
	{
		HRESULT hr = S_OK;
	...
	HRESULT CDxState::SetBlendState(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pContext, D3D11_BLEND_DESC* pDesc, FLOAT BlendFactor[], UINT SampleMask)
	{
		HRESULT hr = S_OK;
	...
	HRESULT CDxState::SetRasterizerState(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pContext, D3D11_RASTERIZER_DESC* pDesc)
	{
		HRESULT hr = S_OK;
	...
	HRESULT CDxState::SetDepthStencilState(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pContext, D3D11_DEPTH_STENCIL_DESC* pDesc, UINT StencilRef)
	{
		HRESULT hr = S_OK;
