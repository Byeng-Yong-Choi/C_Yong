...
void C_Device::SendMSGClick()
{

}

bool C_Device::DeviceInit()
{
	HRESULT hr;
...
HRESULT C_Device::CreateGIFactory()
{
	HRESULT hr = S_OK;
...
HRESULT C_Device::CreateSwapChain()
{
	HRESULT hr = S_OK;
...
HRESULT C_Device::SetRenderTargetView()
{
	HRESULT hr=S_OK;
...
void C_Device::SetViewPort()
{
	D3D11_VIEWPORT vp;									//뷰 포트 정보 입력
	vp.TopLeftX = 0;
	vp.TopLeftY = 0;									//어디에서부터 뿌릴건지에 대한 좌표
	vp.Width = Winrt.right;
	vp.Height = Winrt.bottom;							//얼마만큼 뿌릴지
	vp.MinDepth = 0;
	vp.MaxDepth = 1;									//z값으로 무조건 0~1이다.
	m_pContext->RSSetViewports(1, &vp);					//뷰포트 세팅
}

bool C_Device::ResizeDevice(UINT width, UINT height)
{
	HRESULT hr = S_OK;
...
HRESULT C_Device::CreateDXrc()
{
	HRESULT hr = S_OK;
	return hr;
}

HRESULT C_Device::DeleteDXrc()
{
	HRESULT hr = S_OK;
	return hr;
}
